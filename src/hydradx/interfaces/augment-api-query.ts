...
declare module "@polkadot/api-base/types/storage" {
  interface AugmentedQueries<ApiType extends ApiTypes> {
    ...
    broadcast: {
      ...
      executionContext: AugmentedQuery<
        ApiType,
        () => Observable<Vec<PalletBroadcastExecutionType>>,
        []
      > &
        QueryableStorageEntry<ApiType, []>;
      /** Next available incremental ID */
      incrementalId: AugmentedQuery<ApiType, () => Observable<u32>, []> &
        QueryableStorageEntry<ApiType, []>;
      /**
       * If filled, we overwrite the original swapper. Mainly used in router to
       * not to use temporary trade account
       */
      swapper: AugmentedQuery<
        ApiType,
        () => Observable<Option<AccountId32>>,
        []
      > &
    ...
    circuitBreaker: {
      ...
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      assetLockdownState: AugmentedQuery<
        ApiType,
        (
          arg: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletCircuitBreakerLockdownStatus>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /**
    ...
    dispatcher: {
      aaveManagerAccount: AugmentedQuery<
        ApiType,
        () => Observable<AccountId32>,
        []
      > &
        QueryableStorageEntry<ApiType, []>;
      extraGas: AugmentedQuery<ApiType, () => Observable<u64>, []> &
        QueryableStorageEntry<ApiType, []>;
      lastEvmCallExitReason: AugmentedQuery<
        ApiType,
        () => Observable<Option<EvmCoreErrorExitReason>>,
        []
      > &
        QueryableStorageEntry<ApiType, []>;
      /** Generic query */
    ...
    duster: {
      ...
      accountWhitelist: AugmentedQuery<
        ApiType,
    ...
    dynamicFees: {
      ...
      assetFeeConfiguration: AugmentedQuery<
        ApiType,
        (
          arg: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletDynamicFeesAssetFeeConfig>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /** Generic query */
    ...
    hsm: {
       ...
       *
       * This storage maps asset IDs to their collateral configuration
       * information. Only assets in this map can be used to mint or redeem
       * Hollar through HSM. Each collateral has specific parameters controlling
       * its usage in the HSM mechanism.
       */
      collaterals: AugmentedQuery<
        ApiType,
        (
          arg: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletHsmCollateralInfo>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /** Address of the flash loan receiver. */
      flashMinter: AugmentedQuery<ApiType, () => Observable<Option<H160>>, []> &
        QueryableStorageEntry<ApiType, []>;
      /**
       * Amount of Hollar bought with an asset in a single block
       *
       * This storage tracks how much Hollar has been bought back by HSM for
       * each collateral asset within the current block. This is used to enforce
       * rate limiting on Hollar redemptions. Values are reset to zero at the
       * end of each block in on_finalize.
       */
      hollarAmountReceived: AugmentedQuery<
        ApiType,
    ...
    liquidation: {
      ...
      borrowingContract: AugmentedQuery<ApiType, () => Observable<H160>, []> &
        QueryableStorageEntry<ApiType, []>;
      /** Generic query */
      [key: string]: QueryableStorageEntry<ApiType>;
    };
    ...
    parameters: {
      isTestnet: AugmentedQuery<ApiType, () => Observable<bool>, []> &
        QueryableStorageEntry<ApiType, []>;
      /** Generic query */
      [key: string]: QueryableStorageEntry<ApiType>;
    };
    polkadotXcm: {
      ...
      queryCounter: AugmentedQuery<ApiType, () => Observable<u64>, []> &
        QueryableStorageEntry<ApiType, []>;
      /**
       * If [`ShouldRecordXcm`] is set to true, then the last XCM program
       * executed locally will be stored here. Runtime APIs can fetch the XCM
       * that was executed by accessing this value.
       *
       * Only relevant if this pallet is being used as the
       * [`xcm_executor::traits::RecordXcm`] implementation in the XCM executor
       * configuration.
       */
      ...
      > &
        QueryableStorageEntry<ApiType, []>;
      /**
       * Whether or not incoming XCMs (both executed locally and received)
       * should be recorded. Only one XCM program will be recorded at a time.
       * This is meant to be used in runtime APIs, and it's advised it stays
       * false for all other use cases, so as to not degrade regular performance.
       *
       * Only relevant if this pallet is being used as the
       * [`xcm_executor::traits::RecordXcm`] implementation in the XCM executor
       * configuration.
    ...
    stableswap: {
      ...
      blockFee: AugmentedQuery<
        ApiType,
        (arg: u32 | AnyNumber | Uint8Array) => Observable<Option<Permill>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /** Pool peg info. */
      poolPegs: AugmentedQuery<
        ApiType,
        (
          arg: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletStableswapPoolPegInfo>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /** Existing pools */
      ...
      poolSnapshots: AugmentedQuery<
        ApiType,
        (
          arg: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletStableswapPoolSnapshot>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /** Generic query */
    ...
    staking: {
      ...
      > &
        QueryableStorageEntry<ApiType, [u128]>;
      /**
       * Legacy storage! - Used to handle democracy votes until democracy pallet
       * is fully removed.
       */
      ...
      > &
        QueryableStorageEntry<ApiType, [u128]>;
      /**
       * Legacy storage! - Used to handle democracy processed votes until
       * democracy pallet is fully removed.
       */
      ...
      sixSecBlocksSince: AugmentedQuery<ApiType, () => Observable<u32>, []> &
        QueryableStorageEntry<ApiType, []>;
