...
declare module "@polkadot/api-base/types/consts" {
  interface AugmentedConsts<ApiType extends ApiTypes> {
    ...
    aura: {
      /**
       * The slot duration Aura should run with, expressed in milliseconds. The
       * effective value of this type should not change while the chain is running.
       *
    ...
    messageQueue: {
       ...
       * `on_idle` which should be provided to the message queue for servicing
       * enqueued items `on_idle`. Useful for parachains to process messages at
       * the same block they are received.
       *
       ...
       * queue for servicing enqueued items `on_initialize`.
       *
       * This may be legitimately `None` in the case that you will call
       * `ServiceQueues::service_queues` manually or set
    ...
    xcmpQueue: {
      /**
       * Maximal number of outbound XCMP channels that can have messages queued
       * at the same time.
       *
       * If this is reached, then no further messages can be sent to channels
       * that do not yet have a message queued. This should be set to the
       * expected maximum of outbound channels which is determined by
       * [`Self::ChannelInfo`]. It is important to set this large enough, since
       * otherwise the congestion control protocol will not work as intended and
       * messages may be dropped. This value increases the PoV and should
       * therefore not be picked too high. Governance needs to pay attention to
       * not open more channels than this value.
       */
       ...
       *
       * A lower limit can be set dynamically, but this is the hard-limit for
       * the PoV worst case benchmarking. The limit for the size of a message is
       * slightly below this, since some overhead is incurred for encoding the format.
       */
