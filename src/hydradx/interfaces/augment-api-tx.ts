...
declare module "@polkadot/api-base/types/submittable" {
  interface AugmentedSubmittables<ApiType extends ApiTypes> {
    assetRegistry: {
       ...
       *
       * Asset's id is optional and it can't be used by another asset if it's
       * provided. Provided `asset_id` must be from within reserved range. If
       * `asset_id` is `None`, new asset is given id for sequential ids.
       *
       * Asset's name is optional and it can't be used by another asset if it's
       * provided. Adds mapping between `name` and assigned `asset_id` so asset
       * id can be retrieved by name too (Note: this approach is used in AMM
       * implementation (xyk))
       *
       ...
       *
       * `decimals` - can be update by `UpdateOrigin` only if it wasn't set yet.
       * Only `RegistryOrigin` can update `decimals` if it was previously set.
       *
    ...
    balances: {
       ...
       * result of the burn and `keep_alive` is false, the account will be reaped.
       *
       * Unlike sending funds to a _burn_ address, which merely makes the funds
       * inaccessible, this `burn` operation will reduce total issuance by the
       * amount _burned_.
       */
       ...
       * just in order to allow for the possibility of churn).
       */
    ...
    bonds: {
      /**
       * Issue new fungible bonds. New asset id is registered and assigned to
       * the bonds. The number of bonds the issuer receives is 1:1 to the
       * `amount` of the underlying asset minus the protocol fee. The bond asset
       * is registered with the empty string for the asset name, and with the
       * same existential deposit as of the underlying asset. Bonds can be
       * redeemed for the underlying asset once mature. Protocol fee is applied
       * to the amount, and transferred to `T::FeeReceiver`. When issuing new
       * bonds with the underlying asset and maturity that matches existing
       * bonds, new amount of these existing bonds is issued, instead of
       * registering new bonds. It's possible to issue new bonds for bonds that
       * are already mature.
       *
       * Parameters:
       *
       ...
       *
       * Emits `BondTokenCreated` event when successful and new bonds were
       * registered. Emits `BondsIssued` event when successful.
       */
      ...
      >;
      /**
       * Redeem bonds for the underlying asset. The amount of the underlying
       * asset the `origin` receives is 1:1 to the `amount` of the bonds. Anyone
       * who holds the bonds is able to redeem them. Bonds can be both partially
       * or fully redeemed.
       *
       * Parameters:
       *
    ...
    circuitBreaker: {
      addEgressAccounts: AugmentedSubmittable<
        (
       ...
       */
      forceLiftLockdown: AugmentedSubmittable<
        (
          assetId: u32 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u32]
       ...
       */
      lockdownAsset: AugmentedSubmittable<
        (
          assetId: u32 | AnyNumber | Uint8Array,
          until: u32 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u32, u32]
      >;
      /**
      ...
      releaseDeposit: AugmentedSubmittable<
        ...
        [AccountId32, u32]
      >;
      removeEgressAccounts: AugmentedSubmittable<
        (
          accounts: Vec<AccountId32> | (AccountId32 | string | Uint8Array)[]
        ) => SubmittableExtrinsic<ApiType>,
        [Vec<AccountId32>]
      >;
      /**
       * Reset the global lockdown and accumulator to zero at current block. Can
       * be called only by authority origin.
       */
      resetWithdrawLockdown: AugmentedSubmittable<
        () => SubmittableExtrinsic<ApiType>,
        []
      >;
      /**
      ...
      >;
      setAssetCategory: AugmentedSubmittable<
        (
          assetId: u32 | AnyNumber | Uint8Array,
          category:
            | Option<PalletCircuitBreakerGlobalAssetCategory>
            | null
            | Uint8Array
            | PalletCircuitBreakerGlobalAssetCategory
            | "External"
        ...
        [u32, Option<PalletCircuitBreakerGlobalAssetCategory>]
      >;
      /**
       * Set the global withdraw limit (reference currency units) Can be called
       * only by authority origin.
       */
      setGlobalWithdrawLimitParams: AugmentedSubmittable<
        (
          parameters:
            | PalletCircuitBreakerGlobalWithdrawLimitParameters
            | { limit?: any; window?: any }
            | string
            | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [PalletCircuitBreakerGlobalWithdrawLimitParameters]
      >;
      setGlobalWithdrawLockdown: AugmentedSubmittable<
        (until: u64 | AnyNumber | Uint8Array) => SubmittableExtrinsic<ApiType>,
        [u64]
      >;
      /**
    ...
    collatorSelection: {
      /**
       * Add a new account `who` to the list of `Invulnerables` collators. `who`
       * must have registered session keys. If `who` is a candidate, they will
       * be removed.
       *
       ...
       *
       * If the candidacy bond is increased by this call, all current candidates
       * which have a deposit lower than the new bond will be kicked from the
       * list and get their deposits back.
       *
       * The origin for this call must be the `UpdateOrigin`.
       */
      ...
      >;
      /**
       * Set the ideal number of non-invulnerable collators. If lowering this
       * number, then the number of running collators could be higher than this
       * figure. Aside from that edge case, there should be no other way to have
       * more candidates than the desired number.
       *
       * The origin for this call must be the `UpdateOrigin`.
       */
      ...
      >;
      /**
       * Set the list of invulnerable (fixed) collators. These collators must do
       * some preparation, namely to have registered session keys.
       *
       * The call will remove any accounts that have not registered keys from
       * the set. That is, it is non-atomic; the caller accepts all `AccountId`s
       * passed in `new` _individually_ as acceptable Invulnerables, and is not
       * proposing a _set_ of new Invulnerables.
       *
       * This call does not maintain mutual exclusivity of `Invulnerables` and
       * `Candidates`. It is recommended to use a batch of `add_invulnerable`
       * and `remove_invulnerable` instead. A `batch_all` can also be used to
       * enforce atomicity. If any candidates are included in `new`, they should
       * be removed with `remove_invulnerable_candidate` after execution.
       *
       ...
       * candidate list by reserving `deposit`. The amount `deposit` reserved by
       * the caller must be greater than the existing bond of the target it is
       * trying to replace.
       *
       * This call will fail if the caller is already a collator candidate or
       * invulnerable, the caller does not have registered session keys, the
       * target is not a collator candidate, and/or the `deposit` amount cannot
       * be reserved.
       */
      ...
      >;
      /**
       * Update the candidacy bond of collator candidate `origin` to a new
       * amount `new_deposit`.
       *
       * Setting a `new_deposit` that is lower than the current deposit while
       * `origin` is occupying a top-`DesiredCandidates` slot is not allowed.
       *
       * This call will fail if `origin` is not a collator candidate, the
       * updated bond is lower than the minimum candidacy bond, and/or the
    ...
    convictionVoting: {
       ...
       * Emits `Delegated`.
       *
       * Weight: `O(R)` where R is the number of polls the voter delegating to
       * has voted on. Weight is initially charged as if maximum votes, but is
       * refunded later.
       */
       ...
       */
      forceRemoveVote: AugmentedSubmittable<
        (
          target: AccountId32 | string | Uint8Array,
          clazz: u16 | AnyNumber | Uint8Array,
       ...
       * must have expired, either because the poll was cancelled, because the
       * voter lost the poll or because the conviction period is over.
       *
       * The dispatch origin of this call must be _Signed_.
       *
       * - `target`: The account of the vote to be removed; this account must have
       ...
       *   poll. For polls which have finished or are cancelled, this must be `Some`.
       *
       * Weight: `O(R + log R)` where R is the number of polls that `target` has
       * voted on. Weight is calculated for the maximum number of vote.
       */
      ...
      >;
      /**
       * Undelegate the voting power of the sending account for a particular
       * class of polls.
       *
    ...
    dca: {
      /**
       * Creates a new DCA (Dollar-Cost Averaging) schedule and plans the next
       * execution for the specified block.
       *
       * If the block is not specified, the execution is planned for the next
       * block. If the given block is full, the execution will be planned in the
       * subsequent block.
       *
       * Once the schedule is created, the specified `total_amount` will be
       * reserved for DCA. The reservation currency will be the `amount_in`
       * currency of the order.
       *
       * Trades are executed as long as there is budget remaining from the
       * initial `total_amount` allocation, unless `total_amount` is 0, then
       * trades are executed until schedule is terminated.
       *
       * If a trade fails due to slippage limit or price stability errors, it
       * will be retried. If the number of retries reaches the maximum allowed,
       * the schedule will be terminated permanently. In the case of a
       * successful trade, the retry counter is reset.
       *
       * Parameters:
       *
      ...
      >;
      /**
       * Unlocks DCA reserves of provided asset for the caller if they have no
       * active schedules.
       *
       * This is a utility function to help users recover their reserved funds
       * in case a DCA schedule was terminated but left some reserved amounts.
       *
       ...
       */
      unlockReserves: AugmentedSubmittable<
        (
          who: AccountId32 | string | Uint8Array,
          assetId: u32 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
    ...
    democracy: {
       ...
       * Allow to force remove a vote for a referendum.
       *
       * Same as `remove_other_vote`, except the scope is overriden by forced
       * flag. The dispatch origin of this call must be `VoteRemovalOrigin`.
       *
       * Only allowed if the referendum is finished.
       *
       * The dispatch origin of this call must be _Signed_.
    ...
    dispatcher: {
      dispatchAsAaveManager: AugmentedSubmittable<
        (
          call: Call | IMethod | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [Call]
      >;
      dispatchAsTreasury: AugmentedSubmittable<
        (
          call: Call | IMethod | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [Call]
      >;
      /**
       * Execute a single EVM call. This extrinsic will fail if the EVM call
       * returns any other ExitReason than `ExitSucceed(Returned)` or
       * `ExitSucceed(Stopped)`. Look the
       * [hydradx_runtime::evm::runner::WrapRunner] implementation for details.
       *
       * Parameters:
       *
       ...
       */
      dispatchEvmCall: AugmentedSubmittable<
        (
          call: Call | IMethod | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [Call]
       ...
       *
       * This allows executing calls with additional weight (gas) limit. The
       * extra gas is not refunded, even if not used.
       */
      dispatchWithExtraGas: AugmentedSubmittable<
        (
          call: Call | IMethod | string | Uint8Array,
          extraGas: u64 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [Call, u64]
      >;
      /**
       ...
       *
       * This is intented to be mainly used in testnet environments, where the
       * manager account can be different.
       */
      noteAaveManager: AugmentedSubmittable<
        (
    ...
    duster: {
       ...
       * remaining balance is transferred to treasury account.
       *
       * In case of AToken, we perform an erc20 dust, which does a wihtdraw all
       * to the treasury account Note that in this case, the treasury will just
       * receive the underlying token, not the atoken variant.
       *
      ...
      >;
      /**
       * Remove account from list of whitelist accounts. That means account can
       * be dusted again.
       *
      ...
      >;
      /**
       * Add account to list of whitelist accounts. Account which are excluded
       * from dusting. If such account should be dusted - `AccountWhitelisted`
       * error is returned. Only root can perform this action.
       *
       * Emits `Added` event when successful.
       */
      whitelistAccount: AugmentedSubmittable<
        (
          account: AccountId32 | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [AccountId32]
    ...
    dynamicFees: {
       ...
       *
       * This function removes any custom fee configuration for the specified
       * asset. After removal, the asset will use the default dynamic fee
       * parameters configured in the runtime.
       *
       * # Arguments
       *
       ...
       */
      removeAssetFee: AugmentedSubmittable<
        (
          assetId: u32 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u32]
       ...
       *
       * This function allows setting either fixed or dynamic fee configuration
       ...
       */
      setAssetFee: AugmentedSubmittable<
        (
          assetId: u32 | AnyNumber | Uint8Array,
          config:
            | PalletDynamicFeesAssetFeeConfig
        ...
        [u32, PalletDynamicFeesAssetFeeConfig]
      >;
      /** Generic tx */
      [key: string]: SubmittableExtrinsicFunction<ApiType>;
    };
    emaOracle: {
      ...
      removeOracle: AugmentedSubmittable<
        ...
        [U8aFixed, ITuple<[u32, u32]>]
      >;
      updateBifrostOracle: AugmentedSubmittable<
        (
          assetA:
            | XcmVersionedLocation
            | { V3: any }
        ...
        [XcmVersionedLocation, XcmVersionedLocation, ITuple<[u128, u128]>]
      >;
      /** Generic tx */
      [key: string]: SubmittableExtrinsicFunction<ApiType>;
    };
    ethDispenser: {
       ...
       * otherwise the call will fail with `InvalidRequestId`. Parameters:
       *
       ...
       */
      requestFund: AugmentedSubmittable<
        (
          to: H160 | string | Uint8Array,
          amount: u128 | AnyNumber | Uint8Array,
          requestId: U8aFixed | string | Uint8Array,
          tx:
            | PalletDispenserEvmTransactionParams
            | {
                value?: any;
        ...
        [H160, u128, U8aFixed, PalletDispenserEvmTransactionParams]
      >;
      /**
       * Increase the tracked faucet ETH balance (in wei).
       *
       * This is an accounting helper used to keep `FaucetBalanceWei` roughly in
       * sync with the real faucet balance on the EVM chain.
       *
       * Parameters:
       *
       * - `origin`: Must satisfy `UpdateOrigin`.
       ...
       */
      setFaucetBalance: AugmentedSubmittable<
        (
          balanceWei: u128 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u128]
      >;
      /**
    ...
    evmAccounts: {
      /**
       * Adds an EVM address to the list of addresses that are allowed to deploy
       * smart contracts.
       *
       * Parameters:
       *
      ...
      >;
      /**
       * Adds address of the contract to the list of approved contracts to
       * manage balances.
       *
       ...
       */
      approveContract: AugmentedSubmittable<
        (address: H160 | string | Uint8Array) => SubmittableExtrinsic<ApiType>,
        [H160]
      >;
      /**
       * Binds a Substrate address to EVM address. After binding, the EVM is
       * able to convert an EVM address to the original Substrate address.
       * Without binding, the EVM converts an EVM address to a truncated
       * Substrate address, which doesn't correspond to the origin address.
       *
      ...
      >;
      /**
       * Proves ownership of an account and binds it to the EVM address. This is
       * useful for accounts that want to submit some substrate transaction, but
       * only received some ERC20 balance and `System` pallet doesn't register
       * them as a substrate account.
       *
       * Parameters:
       *
       ...
       */
      claimAccount: AugmentedSubmittable<
        (
          account: AccountId32 | string | Uint8Array,
          assetId: u32 | AnyNumber | Uint8Array,
          signature:
        ...
        [AccountId32, u32, SpRuntimeMultiSignature]
      >;
      /**
       * Removes address of the contract from the list of approved contracts to
       * manage balances.
       *
       * Parameters:
       *
       ...
       */
      disapproveContract: AugmentedSubmittable<
        (address: H160 | string | Uint8Array) => SubmittableExtrinsic<ApiType>,
        [H160]
      >;
      /**
       * Removes an EVM address from the list of addresses that are allowed to
       * deploy smart contracts.
       *
       * Parameters:
       *
       * - `origin`: Substrate account removing the EVM address from the
       *   whitelist. Must be `ControllerOrigin`.
      ...
      >;
      /**
       * Removes the account's EVM address from the list of addresses that are
       * allowed to deploy smart contracts. Based on the best practices, this
       * extrinsic can be called by any whitelisted account to renounce their
       * own permission.
       *
       * Parameters:
       *
    ...
    hsm: {
       ...
       *
       * This function adds a new asset as an approved collateral for Hollar.
       * Only callable by the governance (root origin).
       *
       * Parameters:
       *
       * - `origin`: Must be Root
       ...
       * - `purchase_fee`: Fee applied when buying Hollar with this asset (added
       *   to purchase price)
       * - `max_buy_price_coefficient`: Maximum buy price coefficient for HSM to
       *   buy back Hollar
       * - `buy_back_fee`: Fee applied when buying back Hollar (subtracted from buy price)
       * - `buyback_rate`: Parameter that controls how quickly HSM can buy Hollar
       *   with this asset
       ...
       * - `PoolAlreadyHasCollateral` if another asset from the same pool is
       *   already approved
       ...
       */
      addCollateralAsset: AugmentedSubmittable<
        (
          assetId: u32 | AnyNumber | Uint8Array,
          poolId: u32 | AnyNumber | Uint8Array,
          purchaseFee: Permill | AnyNumber | Uint8Array,
          maxBuyPriceCoefficient: u128 | AnyNumber | Uint8Array,
          buyBackFee: Permill | AnyNumber | Uint8Array,
          buybackRate: Perbill | AnyNumber | Uint8Array,
          maxInHolding: Option<u128> | null | Uint8Array | u128 | AnyNumber
        ) => SubmittableExtrinsic<ApiType>,
        [u32, u32, Permill, u128, Permill, Perbill, Option<u128>]
      >;
      /**
       * Buy asset from HSM
       *
       ...
       *
       * The valid pairs must include Hollar as one side and an approved
       * collateral as the other side.
       *
       * Parameters:
       *
       ...
       * - `InvalidEVMInteraction` if there's an error interacting with the Hollar
       *   ERC20 contract
       * - Other errors from underlying calls
       */
      buy: AugmentedSubmittable<
        (
          ...
          slippageLimit: u128 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u32, u32, u128, u128]
      >;
      /**
       ...
       * using flash loans
       *
       * This call is designed to be triggered automatically by offchain
       * workers. It executes arbitrage by taking a flash loan from the GHO
       * contract and performing trades to profit from price imbalances between
       * HSM and the StableSwap pool.
       *
       ...
       *
       * - For HollarIn (buy direction): Sell Hollar to HSM for collateral, then
       *   sell collateral back for Hollar in pool
       * - For HollarOut (sell direction): Sell Hollar for collateral in pool,
       *   then buy Hollar back from HSM
       *
       * 3. Repays the flash loan
       * 4. Any remaining profit (in collateral) is transferred to the
       *    ArbitrageProfitReceiver
       *
       * This helps maintain the peg of Hollar by profiting from and correcting
       * price imbalances. The call is unsigned and should only be executed by
       * offchain workers.
       *
       * Parameters:
       *
       ...
       * - `arbitrage`: Optional arbitrage parameters (direction and amount). If
       *   None, the function will automatically find and calculate the optimal
       ...
       * - Other errors from underlying calls
       */
      executeArbitrage: AugmentedSubmittable<
        (
          collateralAssetId: u32 | AnyNumber | Uint8Array,
          arbitrage:
            | Option<PalletHsmArbitrage>
            | null
            | Uint8Array
            | PalletHsmArbitrage
            | { HollarOut: any }
        ...
        [u32, Option<PalletHsmArbitrage>]
      >;
      /**
       * Remove a collateral asset
       *
       * Removes an asset from the approved collaterals list. Only callable by
       * the governance (root origin). The collateral must have a zero balance
       * in the HSM account before it can be removed.
       *
       * Parameters:
       *
       * - `origin`: Must be Root
       ...
       */
      removeCollateralAsset: AugmentedSubmittable<
        (
          assetId: u32 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u32]
       ...
       *
       * Configures the EVM address of the flash loan contract that will be used
       * for arbitrage operations. This contract must support the ERC-3156 flash
       * loan standard and be trusted to handle flash loans of Hollar tokens.
       *
       * Parameters:
       *
       ...
       */
      setFlashMinter: AugmentedSubmittable<
        (
          flashMinterAddr: H160 | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [H160]
      >;
      /**
       ...
       *
       * Updates the parameters for an existing collateral asset. Only callable
       * by the governance (root origin). Each parameter is optional and only
       * provided parameters will be updated.
       *
       * Parameters:
       *
       * - `origin`: Must be Root
       ...
       * - `AssetNotApproved` if the asset is not a registered collateral
       */
      updateCollateralAsset: AugmentedSubmittable<
        (
          assetId: u32 | AnyNumber | Uint8Array,
          purchaseFee:
            | Option<Permill>
          ...
          maxBuyPriceCoefficient:
            ...
            | u128
            | AnyNumber,
          buyBackFee: Option<Permill> | null | Uint8Array | Permill | AnyNumber,
          buybackRate:
            | Option<Perbill>
            | null
            | Uint8Array
    ...
    identity: {
      /**
       * Accept a given username that an `authority` granted. The call must
       * include the full username, as in `username.suffix`.
       */
      ...
      >;
      /**
       * Add an `AccountId` with permission to grant usernames with a given
       * `suffix` appended.
       *
       * The authority can grant up to `allocation` usernames. To top up the
       * allocation or change the account used to grant usernames, this call can
       * be used with the updated parameters to overwrite the existing configuration.
       */
      ...
      >;
      /**
       * Call with [ForceOrigin](crate::Config::ForceOrigin) privileges which
       * deletes a username and slashes any deposit associated with it.
       */
       ...
       * - `identity`: The hash of the [`IdentityInformationProvider`] for that
       *   the judgement is provided.
       *
      ...
      /**
       * Remove an expired username approval. The username was approved by an
       * authority but never accepted by the user and must now be beyond its
       * expiration. The call must include the full username, as in `username.suffix`.
       */
      ...
      >;
      /**
       * Permanently delete a username which has been unbinding for longer than
       * the grace period. Caller is refunded the fee if the username expired
       ...
       *
       * If `use_allocation` is set, the authority must have a username
       * allocation available to spend. Otherwise, the authority will need to
       * put up a deposit for registering the username.
       *
       ...
       * - When combined with the suffix of the issuing authority be _less than_
      ...
      setUsernameFor: AugmentedSubmittable<
        ...
        [AccountId32, Bytes, Option<SpRuntimeMultiSignature>, bool]
      >;
      /**
       * Start the process of removing a username by placing it in the unbinding
       * usernames map. Once the grace period has passed, the username can be
       * deleted by calling [remove_username](crate::Call::remove_username).
       */
    ...
    lbp: {
       ...
       *
       * Emits `BuyExecuted` when successful. Deprecated. Emits
       * `pallet_broadcast::Swapped` when successful.
       */
      buy: AugmentedSubmittable<
        (
          assetOut: u32 | AnyNumber | Uint8Array,
       ...
       *
       * Emits `SellExecuted` when successful. Deprecated. Emits
       * `pallet_broadcast::Swapped` when successful.
       */
      sell: AugmentedSubmittable<
        (
    ...
    liquidation: {
      /**
       * Liquidates an existing money market position. Can be both signed and unsigned.
       *
       * Performs a flash loan to get funds to pay for the debt. Received
       * collateral is swapped and the profit is transferred to `FeeReceiver`.
       *
       * Parameters:
       *
       * - `origin`: Signed origin.
      ...
      liquidate: AugmentedSubmittable<
        (
          ...
          debtAsset: u32 | AnyNumber | Uint8Array,
          user: H160 | string | Uint8Array,
          debtToCover: u128 | AnyNumber | Uint8Array,
          route:
        ...
        [u32, u32, H160, u128, Vec<HydradxTraitsRouterTrade>]
      >;
      /** Set the borrowing market contract address. */
      setBorrowingContract: AugmentedSubmittable<
    ...
    multiBlockMigrations: {
       ...
       * `HistoricCleared` event. The first time `None` can be used. `limit`
       * must be chosen in a way that will result in a sensible weight.
       */
       ...
       *
       * This process happens automatically on a runtime upgrade. It is in place
       * as an emergency measurement. The cursor needs to be `None` for this to succeed.
       */
      forceOnboardMbms: AugmentedSubmittable<
        () => SubmittableExtrinsic<ApiType>,
        []
      >;
      /**
       * Allows root to set an active cursor to forcefully start/forward the
       * migration process.
       *
       * This is an edge-case version of [`Self::force_set_cursor`] that allows
       * to set the `started_at` value to the next block number. Otherwise this
       * would not be possible, since `force_set_cursor` takes an absolute block
       * number. Setting `started_at` to `None` indicates that the current block
       * number plus one should be used.
       */
      ...
      >;
      /**
       * Allows root to set a cursor to forcefully start, stop or forward the
       * migration process.
       *
       * Should normally not be needed and is only in place as emergency
       * measure. Note that restarting the migration process in this manner will
       * not call the [`MigrationStatusHandler::started`] hook or emit an
       * `UpgradeStarted` event.
       */
    ...
    multiTransactionPayment: {
      ...
      >;
      /**
       * Dispatch EVM permit. The main purpose of this function is to allow EVM
       * accounts to pay for the transaction fee in non-native currency by
       * allowing them to self-dispatch pre-signed permit. The EVM fee is paid
       * in the currency set for the account.
       */
      ...
      >;
      /**
       * Reset currency of the specified account to HDX. If the account is EVM
       * account, the payment currency is reset to WETH. Only selected members
       ...
       *
       * In case of sufficient asset, the chosen currency must be in the list of
       * accepted currencies In case of insufficient asset, the chosen currency
       * must have a XYK pool with DOT
       *
    ...
    omnipool: {
       ...
       *
       * `add_liquidity` adds specified asset amount to Omnipool and in exchange
       ...
       *
       * Add liquidity fails if price difference between spot price and oracle
       * price is higher than allowed by `PriceBarrier`.
       *
       * Parameters:
       *
       ...
       *   already in the pool
       * - `amount`: Amount of asset added to omnipool
       * - `min_shares_limit`: The min amount of delta share asset the user should
       *   receive in the position
       *
       * Emits `LiquidityAdded` event when successful.
       */
       ...
       *
       * Emits `BuyExecuted` event when successful. Deprecated. Emits
       ...
       *
       * Transfer can be executed only if asset is not in Omnipool and pool's
       ...
       * Limit protection is applied.
       *
       * `remove_all_liquidity` removes all shares amount from given PositionId
       * (NFT instance).
       *
       * Asset's tradable state must contain REMOVE_LIQUIDITY flag, otherwise
       * `NotAllowed` error is returned.
       *
       * If all shares from given position are removed, position is destroyed
       * and NFT is burned.
       *
       * Remove all liquidity fails if price difference between spot price and
       * oracle price is higher than allowed by `PriceBarrier`.
       *
       * Dynamic withdrawal fee is applied if withdrawal is not safe. It is
       * calculated using spot price and external price oracle. Withdrawal is
       * considered safe when trading is disabled.
       *
       * Parameters:
       *
       * - `position_id`: The identifier of position which liquidity is entirely
       *   removed from.
       *
       * Emits `LiquidityRemoved` event when successful.
       */
      removeAllLiquidity: AugmentedSubmittable<
        (
       ...
       * and NFT is burned.
       *
       * Remove liquidity fails if price difference between spot price and
       * oracle price is higher than allowed by `PriceBarrier`.
       *
       * Dynamic withdrawal fee is applied if withdrawal is not safe. It is
       * calculated using spot price and external price oracle. Withdrawal is
       ...
       *
       * Remaining shares must belong to protocol, otherwise `SharesRemaining`
       * error is returned.
       *
       * Protocol's liquidity is transferred to the beneficiary account and hub
       * asset amount is burned.
       *
       ...
       *
       * Emits `SellExecuted` event when successful. Deprecated. Emits
       * `pallet_broadcast::Swapped` event when successful.
       */
      sell: AugmentedSubmittable<
        (
       ...
       *
       * When set to `Some(config)`, slip fees are enabled with the given
       * parameters. When set to `None`, slip fees are disabled.
       *
       * Can only be called by `UpdateTradabilityOrigin`.
       *
       * Emits `SlipFeeSet` event.
       */
      setSlipFee: AugmentedSubmittable<
        (
          slipFee:
            | Option<PalletOmnipoolSlipFeeConfig>
            | null
            | Uint8Array
            | PalletOmnipoolSlipFeeConfig
            | { maxSlipFee?: any }
       ...
       *
       * Protocol liquidity is liquidity from sacrificed positions. In order to
       * remove protocol liquidity, we need the know the price of the position
       * at the time of sacrifice. Hence this specific call.
       *
       * Only `AuthorityOrigin` can perform this call.
       *
       * Note that sacrifice position will be deprecated in future. There is no
       * longer a need for that.
       *
       * It works the same way as remove liquidity call, but position is
    ...
    omnipoolLiquidityMining: {
      /**
       * This function allows user to add liquidity then use that shares to join
       * multiple farms.
       *
       * Parameters:
       *
       ...
       *   receive in the position
       *
       * Emits `SharesDeposited` event for the first farm entry Emits
       * `SharesRedeposited` event for each farm entry after the first one
       */
      addLiquidityAndJoinFarms: AugmentedSubmittable<
        (
          farmEntries:
            | Vec<ITuple<[u32, u32]>>
            | [u32 | AnyNumber | Uint8Array, u32 | AnyNumber | Uint8Array][],
          asset: u32 | AnyNumber | Uint8Array,
          amount: u128 | AnyNumber | Uint8Array,
          minSharesLimit: Option<u128> | null | Uint8Array | u128 | AnyNumber
        ) => SubmittableExtrinsic<ApiType>,
        [Vec<ITuple<[u32, u32]>>, u32, u128, Option<u128>]
      >;
      /**
       * This function allows user to add liquidity to stableswap pool, then
       * adding the stable shares as liquidity to omnipool then use that
       * omnipool shares to join multiple farms.
       *
       * If farm entries are not specified (empty vectoo), then the liquidities
       * are still added to the pools
       *
       * Parameters:
       *
       * - `origin`: owner of the omnipool position to deposit into the liquidity mining.
       * - `stable_pool_id`: id of the stableswap pool to add liquidity to.
       * - `stable_asset_amounts`: amount of each asset to be deposited into the
       *   stableswap pool.
       * - `farm_entries`: list of farms to join.
       * - `min_shares_limit`: optional minimum Omnipool shares to receive
       *   (slippage protection). Applies to Omnipool step only. None defaults
       *   to no protection.
       *
       * Emits `LiquidityAdded` events from both pool Emits `SharesDeposited`
       * event for the first farm entry Emits `SharesRedeposited` event for each
       * farm entry after the first one
       */
      addLiquidityStableswapOmnipoolAndJoinFarms: AugmentedSubmittable<
        (
          stablePoolId: u32 | AnyNumber | Uint8Array,
          stableAssetAmounts:
            | Vec<HydradxTraitsStableswapAssetAmount>
            | (
                | HydradxTraitsStableswapAssetAmount
       ...
       *
       * This function will attempt to withdraw shares and claim rewards (if
       * available) from all specified yield farms for a given deposit.
       *
       * Parameters:
       *
       ...
       */
      exitFarms: AugmentedSubmittable<
        (
          depositId: u128 | AnyNumber | Uint8Array,
          yieldFarmIds: Vec<u32> | (u32 | AnyNumber | Uint8Array)[]
        ) => SubmittableExtrinsic<ApiType>,
        [u128, Vec<u32>]
      >;
      /**
       ...
       * `SharesRedeposited` event for each farm entry after the first one
       */
      joinFarms: AugmentedSubmittable<
        (
          farmEntries:
            | Vec<ITuple<[u32, u32]>>
            | [u32 | AnyNumber | Uint8Array, u32 | AnyNumber | Uint8Array][],
      ...
      >;
      /**
       * Remove liquidity from stableswap and omnipool, optionally exiting
       * associated yield farms.
       *
       * This extrinsic reverses the operation performed by
       * `add_liquidity_stableswap_omnipool_and_join_farms`, with optional farm
       * exit to match the optional farm join in the add function.
       *
       * It performs the following steps in order:
       *
       * 1. [OPTIONAL] If deposit_id is provided: Exits from ALL yield farms
       *    associated with the deposit (claiming rewards)
       * 2. Removes liquidity from the omnipool to retrieve stableswap shares
       *    (protected by omnipool_min_limit)
       * 3. Removes liquidity from the stableswap pool to retrieve underlying
       *    assets (protected by stableswap_min_amounts_out)
       *
       * The stabelswap liquidity asset removal strategy is determined by the
       * `min_amounts_out` parameter length:
       *
       ...
       * - `omnipool_min_limit`: The min amount of asset to be removed from
       *   omnipool (slippage protection)
       * - `stableswap_min_amounts_out`: Asset IDs and minimum amounts minimum
       *   amounts of each asset to receive from omnipool.
       * - `deposit_id`: Optional liquidity mining deposit NFT ID. If provided,
       *   exits all farms first.
       *
       * Emits events:
       *
       ...
       */
      removeLiquidityStableswapOmnipoolAndExitFarms: AugmentedSubmittable<
        (
          positionId: u128 | AnyNumber | Uint8Array,
          omnipoolMinLimit: u128 | AnyNumber | Uint8Array,
          stableswapMinAmountsOut:
            | Vec<HydradxTraitsStableswapAssetAmount>
            | (
                | HydradxTraitsStableswapAssetAmount
                | { assetId?: any; amount?: any }
          ...
          depositId: Option<u128> | null | Uint8Array | u128 | AnyNumber
        ) => SubmittableExtrinsic<ApiType>,
        [u128, u128, Vec<HydradxTraitsStableswapAssetAmount>, Option<u128>]
      >;
      /**
    ...
    otc: {
       ...
       * - `order_id`: ID of the order
       *
       * Events: `Filled` event when successful. Deprecated.
       * `pallet_broadcast::Swapped` event when successful.
       */
       ...
       *   ExistentialDepositMultiplier
       *
       * Events: `PartiallyFilled` event when successful. Deprecated.
       * `pallet_broadcast::Swapped` event when successful.
       */
    ...
    otcSettlements: {
       ...
       *
       * Executes a trade between an OTC order and some route. If the OTC order
       * is partially fillable, the extrinsic fails if the existing arbitrage
       * opportunity is not closed or reduced after the trade. If the OTC order
       * is not partially fillable, fails if there is no profit after the trade.
       *
       ...
       * Parameters:
       *
       * - `origin`: Signed or unsigned origin. Unsigned origin doesn't pay the TX
       *   fee, but can be submitted only by a collator.
    ...
    polkadotXcm: {
       ...
       * making this call. The `aliaser` is only authorized until the provided
       * `expiry` block number. The call can also be used for a previously
       * authorized alias in order to update its `expiry` block number.
       *
       * Usually useful to allow your local account to be aliased into from a
       * remote location also under your control (like your account on another chain).
       *
       * WARNING: make sure the caller `origin` (you) trusts the `aliaser`
       * location to act in their/your name. Once authorized using this call,
       * the `aliaser` can freely impersonate `origin` in XCM programs executed
       * on the local chain.
       */
       ...
       * through their local, destination or remote reserve.
       *
       * `assets` must have same reserve location and may not be teleportable to `dest`.
       *
       * - `assets` have local reserve: transfer assets to sovereign account of
       *   destination chain and forward a notification XCM to `dest` to mint
       ...
       *   forward another XCM to `dest` to mint and deposit reserve-based
       ...
       *   `[Parachain(..)]` to send from relay to parachain.
      ...
      >;
      /**
       * Remove all previously authorized `aliaser`s that can alias into the
       * local `origin` making this call.
       */
      ...
      >;
      /**
       * Remove a previously authorized `aliaser` from the list of locations
       * that can alias into the local `origin` making this call.
       */
      ...
      /**
       * Transfer some assets from the local chain to the destination chain
       * through their local, destination or remote reserve, or through teleports.
       *
       * Fee payment on the destination side is made from the asset in the
       * `assets` vector of index `fee_asset_item` (hence referred to as
       * `fees`), up to enough to pay for `weight_limit` of weight. If more
       * weight is needed than `weight_limit`, then the operation will fail and
       * the sent assets may be at risk.
       *
       * `assets` (excluding `fees`) must have same reserve location or
       * otherwise be teleportable to `dest`, no limitations imposed on `fees`.
       *
       ...
       * `dest`. Caller must provide the `assets_transfer_type` to be used for `assets`:
       *
       ...
       *   reserve-based assets to `beneficiary`. Typically the remote `reserve`
       *   is Asset Hub.
       ...
       * identified by `remote_fees_id`. Make sure enough of the specified
       * `remote_fees_id` asset is included in the given list of `assets`.
       * `remote_fees_id` should be enough to pay for `weight_limit`. If more
       * weight is needed than `weight_limit`, then the operation will fail and
       * the sent assets may be at risk.
       *
       ...
       *
       * The caller needs to specify what should happen to the transferred
       * assets once they reach the `dest` chain. This is done through the
       * `custom_xcm_on_dest` parameter, which contains the instructions to
       * execute on `dest` as a final step. This is usually as simple as:
       * `Xcm(vec![DepositAsset { assets: Wild(AllCounted(assets.len())),
       * beneficiary }])`, but could be something more exotic like sending the
       * `assets` even further.
       *
       * - `origin`: Must be capable of withdrawing the `assets` and executing XCM.
       * - `dest`: Destination context for the assets. Will typically be `[Parent,
       *   Parachain(..)]` to send from parachain to parachain, or
       *   `[Parachain(..)]` to send from relay to parachain, or `(parents: 2,
       *   (GlobalConsensus(..), ..))` to send from parachain across a bridge to
    ...
    proxy: {
       ...
       * current values. This can be used by accounts to possibly lower their
       * locked amount.
       *
       * The dispatch origin for this call must be _Signed_.
       *
    ...
    referrals: {
       ...
       *
       * IF there is any asset in the reward pot, all is converted to
       * RewardCurrency first.
       *
       * Reward amount is calculated based on the shares of the signer account.
       *
       * If the signer account is referrer account, total accumulated rewards is
       * updated as well as referrer level if reached.
       *
       ...
       *
       * `origin` pays the registration fee. `code` is assigned to the given `account`.
       *
       * Length of the `code` must be at least `T::MinCodeLength`. Maximum
       * length is limited to `T::CodeLength`. `code` must contain only
       * alfa-numeric characters and all characters will be converted to upper case.
       *
       * Parameters:
       *
    ...
    router: {
       ...
       *   and the AMM (`pool`) in which the trade is executed. If not
       *   specified, than the on-chain route is used. If no on-chain is
       *   present, then omnipool route is used as default
       *
      ...
      >;
      /**
       * Force inserts the on-chain route for a given asset pair, so there is no
       * any validation for the route
       *
       * Can only be called by T::ForceInsertOrigin
       *
       * The route is stored in an ordered manner, based on the oder of the ids
       * in the asset pair.
       *
       ...
       * - `new_route`: Series of [`Trade<AssetId>`] to be executed. A
       *   [`Trade<AssetId>`] specifies the asset pair (`asset_in`, `asset_out`)
      ...
      >;
      /**
       * Executes a sell with a series of trades specified in the route. It
       * sells all reducible user balance of `asset_in` The price for each trade
       * is determined by the corresponding AMM.
       *
       * - `origin`: The executor of the trade
       * - `asset_in`: The identifier of the asset to sell
       * - `asset_out`: The identifier of the asset to receive
      ...
      sellAll: AugmentedSubmittable<
        ...
        [u32, u32, u128, Vec<HydradxTraitsRouterTrade>]
      >;
      /**
       ...
       *
       * If there is no route explicitly set for an asset pair, then we use the
       * omnipool route as default.
       *
       * When a new route is set, we compare it to the existing (or default)
       * route. The comparison happens by calculating sell amount_outs for the
       * routes, but also for the inversed routes.
       *
       * The route is stored in an ordered manner, based on the oder of the ids
       * in the asset pair.
       *
    ...
    scheduler: {
       ...
       *
       * Tasks which need to be scheduled for a retry are still subject to
       * weight metering and agenda space, same as a regular task. If a periodic
       * task fails, it will be scheduled normally while the task is retrying.
       *
       * Tasks scheduled as a result of a retry for a periodic task are unnamed,
       * non-periodic clones of the original task. Their retry configuration
       * will be derived from the original task's configuration, but will have a
       * lower value for `remaining` than the original `total_retries`.
       */
      ...
      >;
      /**
       * Set a retry configuration for a named task so that, in case its
       * scheduled run fails, it will be retried after `period` blocks, for a
       * total amount of `retries` retries or until it succeeds.
       *
       * Tasks which need to be scheduled for a retry are still subject to
       * weight metering and agenda space, same as a regular task. If a periodic
       * task fails, it will be scheduled normally while the task is retrying.
    ...
    signet: {
      ...
      initialize: AugmentedSubmittable<
        (
          ...
          signatureDeposit: u128 | AnyNumber | Uint8Array,
          chainId: Bytes | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [AccountId32, u128, Bytes]
      >;
      /** Respond to signature requests (batch support) */
      respond: AugmentedSubmittable<
        (
          requestIds: Vec<U8aFixed>,
          signatures:
            | Vec<PalletSignetSignature>
            | (
                | PalletSignetSignature
                | { bigR?: any; s?: any; recoveryId?: any }
                | string
                | Uint8Array
              )[]
        ) => SubmittableExtrinsic<ApiType>,
        [Vec<U8aFixed>, Vec<PalletSignetSignature>]
      >;
      /** Provide a read response with signature */
      respondBidirectional: AugmentedSubmittable<
        (
          requestId: U8aFixed | string | Uint8Array,
          serializedOutput: Bytes | string | Uint8Array,
          signature:
            | PalletSignetSignature
            | { bigR?: any; s?: any; recoveryId?: any }
            | string
        ...
        [U8aFixed, Bytes, PalletSignetSignature]
      >;
      /** Report signature generation errors (batch support) */
      respondError: AugmentedSubmittable<
        (
          errors:
            | Vec<PalletSignetErrorResponse>
            | (
                | PalletSignetErrorResponse
                | { requestId?: any; errorMessage?: any }
                | string
                | Uint8Array
              )[]
        ) => SubmittableExtrinsic<ApiType>,
      ...
      sign: AugmentedSubmittable<
        (
          payload: U8aFixed | string | Uint8Array,
          keyVersion: u32 | AnyNumber | Uint8Array,
          path: Bytes | string | Uint8Array,
          algo: Bytes | string | Uint8Array,
          dest: Bytes | string | Uint8Array,
          params: Bytes | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [U8aFixed, u32, Bytes, Bytes, Bytes, Bytes]
      >;
      /** Request a signature for a serialized transaction */
      signBidirectional: AugmentedSubmittable<
        (
          serializedTransaction: Bytes | string | Uint8Array,
          caip2Id: Bytes | string | Uint8Array,
          keyVersion: u32 | AnyNumber | Uint8Array,
          path: Bytes | string | Uint8Array,
          algo: Bytes | string | Uint8Array,
          dest: Bytes | string | Uint8Array,
          params: Bytes | string | Uint8Array,
          outputDeserializationSchema: Bytes | string | Uint8Array,
          respondSerializationSchema: Bytes | string | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [Bytes, Bytes, u32, Bytes, Bytes, Bytes, Bytes, Bytes, Bytes]
      >;
      /** Update the signature deposit amount (admin only) */
      updateDeposit: AugmentedSubmittable<
        (
          newDeposit: u128 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u128]
      ...
      withdrawFunds: AugmentedSubmittable<
        (
          recipient: AccountId32 | string | Uint8Array,
          amount: u128 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
    ...
    stableswap: {
       ...
       *
       * First call of `add_assets_liquidity` must provide "initial liquidity"
       * of all assets.
       *
       * If there is liquidity already in the pool, LP can provide liquidity of
       * any number of pool assets.
       *
       ...
       *
       * Emits `LiquidityAdded` event when successful. Emits
       * `pallet_broadcast::Swapped` event when successful.
       */
      addAssetsLiquidity: AugmentedSubmittable<
        (
          poolId: u32 | AnyNumber | Uint8Array,
          assets:
            | Vec<HydradxTraitsStableswapAssetAmount>
          ...
          minShares: u128 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u32, Vec<HydradxTraitsStableswapAssetAmount>, u128]
      >;
      /**
       ...
       *
       * This functionality is used mainly by on-chain routing when a swap
       * between Omnipool asset and stable asset is performed.
       *
       * Parameters:
       *
       * - `origin`: liquidity provider
       ...
       *
       * All assets must be correctly registered in `T::AssetRegistry`. Note
       * that this does not seed the pool with liquidity. Use
       * `add_assets_liquidity` to provide initial liquidity.
       *
       * Parameters:
       *
       ...
       *
       * This function allows the creation of a new stable pool with specified
       * assets, amplification, fee, and peg sources. The pool is identified by
       * a share asset.
       *
       * Peg target price is determined by retrieving the target peg from the
       * oracle - it is the price of the asset from the peg sourcedenominated in
       * the other pool assets.
       *
       * Parameters:
       *
       * - `origin`: Must be `T::AuthorityOrigin`.
       ...
       *
       * Emits `PoolCreated` event if successful. Emits `AmplificationChanging`
       * event if successful.
       *
       * # Errors
       *
       ...
       * - `AssetNotRegistered`: If one or more assets are not registered in the
       *   AssetRegistry.
       ...
       */
      createPoolWithPegs: AugmentedSubmittable<
        (
          shareAsset: u32 | AnyNumber | Uint8Array,
          assets: Vec<u32> | (u32 | AnyNumber | Uint8Array)[],
          amplification: u16 | AnyNumber | Uint8Array,
          ...
          maxPegUpdate: Perbill | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u32, Vec<u32>, u16, Permill, Vec<PalletStableswapPegSource>, Perbill]
      >;
      /**
       * Remove liquidity from a selected pool uniformly.
       *
       * This function allows a liquidity provider to withdraw liquidity from a
       * pool. The provider specifies the amount of shares to burn and the
       ...
       * - `min_amounts_out`: A bounded vector specifying the minimum amounts of
       *   each asset to receive.
       *
       * Emits `LiquidityRemoved` event when successful. Emits
       * `pallet_broadcast::Swapped` event when successful.
       *
       * # Errors
       *
       ...
       * - `IncorrectAssets`: If the provided `min_amounts_out` does not match the
       *   pool assets.
       * - `NotAllowed`: If the asset is not allowed for the operation.
       * - `SlippageLimit`: If the amount received is less than the specified
       *   minimum amount.
       * - `InsufficientLiquidityRemaining`: If the remaining liquidity in the
       *   pool is below the minimum required.
       *
       * # Invariants
       *
      ...
      removeLiquidity: AugmentedSubmittable<
        ...
        [u32, u128, Vec<HydradxTraitsStableswapAssetAmount>]
      >;
      /**
       ...
       *
       * This function allows updating the tradability state of an asset within
       * a pool. The tradability state determines whether the asset can be used
       * for specific operations such as adding liquidity, removing liquidity,
       * buying, or selling.
       *
       * Parameters:
       *
       ...
       * - `start_block`: block number when the amplification starts to move
       *   towards final_amplication
       ...
       *
       * This function allows updating the peg source for an asset within a
       * pool. The pool must exist and have pegs configured. The asset must be
       * part of the pool. The current price is always preserved when updating
       * the peg source.
       *
       * Parameters:
       *
       ...
       * - `AssetNotInPool`: If the specified asset is not part of the pool.
       */
      updateAssetPegSource: AugmentedSubmittable<
        (
          poolId: u32 | AnyNumber | Uint8Array,
          assetId: u32 | AnyNumber | Uint8Array,
          pegSource:
        ...
        [u32, u32, PalletStableswapPegSource]
      >;
      /**
       * Update pool's fee.
       *
       ...
       *
       * This function allows updating the maximum percentage by which peg
       * values can change in a pool with pegs configured.
       *
       * Parameters:
       *
       * - `origin`: Must be `T::AuthorityOrigin`.
       ...
       * - `NoPegSource`: If the pool does not have pegs configured.
       */
      updatePoolMaxPegUpdate: AugmentedSubmittable<
        (
          poolId: u32 | AnyNumber | Uint8Array,
          maxPegUpdate: Perbill | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
      ...
      >;
      /**
       * Remove liquidity from selected pool by specifying exact amount of asset
       * to receive.
       *
       * Similar to `remove_liquidity_one_asset` but LP specifies exact amount
       * of asset to receive instead of share amount.
       *
       * Parameters:
       *
       * - `origin`: liquidity provider
    ...
    staking: {
       ...
       *
       * Function calculates amount of rewards to pay for specified staking
       * position based on the amount of points position accumulated. Function
       * also unlocks all the rewards locked from `increase_stake` based on the
       * amount of the points.
       *
       * This action is penalized by removing all the points and returning
       * allocated unpaid rewards for redistribution.
       *
       * Parameters:
       *
      ...
      >;
      /**
       * Extrinsic to increase staked amount of existing staking position by
       * specified `amount`.
       *
       * `increase_stake` increases staked amount of position specified by
       * `postion_id` by the `amount` specified by the user. Staking position
       * must exist and `origin` has to be the owner of the position. Users can
       * stake tokens which are not vested or already staked. Position's params
       * e.g points are updated to offset stake increase and rewards accumulated
       * until this point are paid and locked to the user.
       *
       * Parameters:
       *
       * - `position_id`: The identifier of the position which stake will be increased.
       * - `amount`: Amount of native asset to be added to staked amount. `amount`
       *   can't be vested or already staked
       *
      ...
      >;
      /**
       * Staking pallet initialization. This call will reserved `pot`'s balance
       * to prevent account dusting and start collecting fees from trades as rewards.
       *
       * `pot`’s account has to have a balance which will be reserved to prevent
       * account dusting.
       *
       ...
       *
       * `stake` locks specified `amount` into staking and creates new NFT
       * representing staking position. Users can stake `NativeAssetId` balance
       * which is not vested or already staked.
       *
       * Staking pallet must be initialized otherwise extrinsic will fail with error.
       *
       * Parameters:
       *
       * - `amount`: Amount of native asset to be staked. `amount` can't be vested
       *   or already staked
       *
      ...
      >;
      /**
       * Function pays rewards, unlocks all the staked assets and destroys
       * staking position specified by `position_id`.
       *
       * Function calculates and pays latest rewards, unlocks all the locked
       * rewards and staked tokens for staking position and burns NFT
       * representing staking position. Unpaid allocated rewards are returned to
       * the Staking for redistribution.
       *
       * Parameters:
       *
    ...
    stateTrieMigration: {
       ...
       *
       * The (potentially over-estimated) of the byte length of all the data
       * read must be provided for up-front fee-payment and weighing. In
       * essence, the caller is guaranteeing that executing the current
       * `MigrationTask` with the given `limits` will not exceed
       ...
       * [`MigrationTask::migrate_until_exhaustion`], the recommended way of
       * doing this is to pass a `limit` that only bounds `count`, as the `size`
       ...
       *
       * This is only useful in one case: the next key to migrate is too big to
       * be migrated with a signed account, in a parachain context, and we
       * simply want to skip it. A reasonable example of this would be `:code:`,
       * which is both very expensive to migrate, and commonly used, so probably
       * it is already migrated.
       *
       * In case you mess things up, you can also, in principle, use this to
    ...
    technicalCommittee: {
       ...
       *
       * Emits `Killed` and `ProposalCostBurned` if any cost was held for a
       * given proposal.
       */
      kill: AugmentedSubmittable<
        (
      ...
      >;
      /**
       * Release the cost held for storing a proposal once the given proposal is
       * completed.
       *
       * If there is no associated cost for the given proposal, this call will
       * have no effect.
       *
       * Parameters:
       *
    ...
    treasury: {
       ...
       * ## Details
       *
       * The status check is a prerequisite for retrying a failed payout. If a
       * spend has either succeeded or expired, it is removed from the storage
       * by this function. In such instances, transaction fees are refunded.
       *
       * ### Parameters
       *
       ...
       * claimed within one [`Config::PayoutPeriod`] from the `valid_from`
       * block. In case of a payout failure, the spend status must be updated
       * with the `check_status` dispatchable before retrying with the current function.
       *
       * ### Parameters
       *
       * - `index`: The spend index.
       ...
       * ### Errors
       *
       * - [`Error::ProposalNotApproved`]: The `proposal_id` supplied was not
       *   found in the approval queue, i.e., the proposal has not been
       *   approved. This could also mean the proposal does not exist
       *   altogether, thus there is no way it would have been approved in the
       *   first place.
       */
       ...
       * `amount` of `asset_kind` in the native asset. The amount of
       * `asset_kind` is converted for assertion using the [`Config::BalanceConverter`].
       *
       * ## Details
       *
       * Create an approved spend for transferring a specific `amount` of
       * `asset_kind` to a designated beneficiary. The spend must be claimed
       * using the `payout` dispatchable within the [`Config::PayoutPeriod`].
       *
       * ### Parameters
       *
       ...
       *   according to the [`Config::PayoutPeriod`]. If `None`, the spend can
       *   be claimed immediately after approval.
       *
       * ## Events
       *
    ...
    utility: {
       ...
       * Dispatches a function call with a provided origin.
       *
       * Almost the same as [`Pallet::dispatch_as`] but forwards any error of
       * the inner call.
       *
       * The dispatch origin for this call must be _Root_.
       */
       ...
       * May be called from any origin except `None`.
       *
       * This function first attempts to dispatch the `main` call. If the `main`
       * call fails, the `fallback` is attemted. if the fallback is successfully
       * dispatched, the weights of both calls are accumulated and an event
       * containing the main call error is deposited.
       *
       * In the event of a fallback failure the whole call fails with the
       * weights returned.
       *
       ...
       * ## Dispatch Logic
       *
       * - If the origin is `root`, both the main and fallback calls are executed
       *   without applying any origin filters.
    ...
    xTokens: {
       ...
       * requirements, the execution may fail and assets wouldn't be received.
       *
       * `fee` is the Asset to be spent to pay for execution in destination
       * chain. Both fee and amount will be subtracted form the callers balance
       * For now we only accept fee and asset having the same `Location` id.
       *
    ...
    xyk: {
      ...
      addLiquidity: AugmentedSubmittable<
        ...
        [u32, u32, u128, u128]
      >;
      addLiquidityWithLimits: AugmentedSubmittable<
        (
          assetA: u32 | AnyNumber | Uint8Array,
          assetB: u32 | AnyNumber | Uint8Array,
          amountA: u128 | AnyNumber | Uint8Array,
       ...
       * `asset_out`. Emits `BuyExecuted` when successful. Deprecated. Emits
       * `pallet_broadcast::Swapped` when successful.
       */
      buy: AugmentedSubmittable<
        (
      ...
      removeLiquidity: AugmentedSubmittable<
        ...
        [u32, u32, u128]
      >;
      removeLiquidityWithLimits: AugmentedSubmittable<
        (
          assetA: u32 | AnyNumber | Uint8Array,
          assetB: u32 | AnyNumber | Uint8Array,
          shareAmount: u128 | AnyNumber | Uint8Array,
          minAmountA: u128 | AnyNumber | Uint8Array,
          minAmountB: u128 | AnyNumber | Uint8Array
        ) => SubmittableExtrinsic<ApiType>,
        [u32, u32, u128, u128, u128]
      >;
      /**
    ...
    xykLiquidityMining: {
       ...
       *
       * The share is deposited to the first farm of the specified entries, and
       * then redeposit the shares to the remaining farms
       *
       * Parameters:
       *
      ...
      addLiquidityAndJoinFarms: AugmentedSubmittable<
        ...
        [u32, u32, u128, u128, Vec<ITuple<[u32, u32]>>]
      >;
      /**
       ...
       * will be transferred from `owner` to farm account. In case of
       * `reward_currency` is insufficient asset, farm's `owner` has to pay
       * existential deposit for global farm account and for liquidity mining
       * `pot` account.
       *
       * The dispatch origin for this call must be `T::CreateOrigin`. !!!WARN:
       * `T::CreateOrigin` has power over funds of `owner`'s account and it
      ...
      exitFarms: AugmentedSubmittable<
        (
          ...
          assetPair:
            ...
            | string
            | Uint8Array,
          farmEntries: Vec<u32> | (u32 | AnyNumber | Uint8Array)[]
        ) => SubmittableExtrinsic<ApiType>,
        [u128, PalletXykAssetPair, Vec<u32>]
      >;
      /**
       * Join multiple farms with a given share amount
       *
       * The share is deposited to the first farm of the specified fams, and
       * then redeposit the shares to the remaining farms
       *
       * Parameters:
       *
      ...
      joinFarms: AugmentedSubmittable<
        ...
        [Vec<ITuple<[u32, u32]>>, PalletXykAssetPair, u128]
      >;
      /**
